...
When the bundled local application module is running, Fredy discovers it automatically on localhost; no HTTP channel needs to be created or attached to a job. **Settings → Auto-apply** can define one simple AND rule using jobs, price, size, rooms and travel times to saved places. Before queuing Selenium, Fredy compares WBS wording from structured provider data or the listing text with the WBS type stored in the application-module profile and also checks the two currently supported hard eligibility facts, **special housing need** and **age 55+**. Only a known incompatibility blocks auto-apply; ambiguous income or eligibility wording remains fail-open. Fredy checks which provider workflows currently exist before acting: a matching listing with a workflow is queued immediately, while every other listing still reaches Telegram. If a workflow exists, a non-automatic Telegram notification always carries a one-tap **Apply** button, so manual approval can override an automatic eligibility guard. Application progress is stored on the listing and reflected back into the original Telegram message. An explicitly configured application HTTP channel remains supported as an override.

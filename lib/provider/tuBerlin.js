 ...
 * @returns {boolean}
 */
function applyBlacklist(listing, blacklist = [], blacklistedDistricts = []) {
  ...
  const districtBlacklisted = isOneOf(listing.description, blacklistedDistricts);
  return (
    listing.title != null &&
    titleNotBlacklisted &&
...
/**
 * Build a run-scoped provider configuration for a Fredy job.
 *
...
export const createConfig = (sourceConfig, blacklist = []) => {
  const blacklistedDistricts = Array.isArray(sourceConfig.blacklistedDistricts)
    ? sourceConfig.blacklistedDistricts
    : [];
  return {
    ...config,

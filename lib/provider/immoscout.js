...
const SCOUT_OFFICIAL_PROVIDER_BY_COMPANY = new Map(
  [
    ...
    ['WBM', 'wbm'],
  ].map(([name, providerId]) => [name.toLocaleLowerCase('de-DE').replace(/\s+/g, ' ').trim(), providerId]),
);

/**
 * Map the structured ImmoScout advertiser name to the provider id shared with the direct-link
 * matcher and the application module. Unknown companies deliberately stay unknown.
 *
 * @param {unknown} company
 * @returns {string|null}
 */
export function officialProviderFromScoutCompany(company) {
  if (typeof company !== 'string') return null;
  const key = company.toLocaleLowerCase('de-DE').replace(/\s+/g, ' ').trim();
  return SCOUT_OFFICIAL_PROVIDER_BY_COMPANY.get(key) ?? null;
}
...
async function pushDetails(listing) {
  ...
  const travelTime = (detailBody?.sections || []).find(
    (section) => section?.type === 'TRAVELTIME' && section?.isBlocked === false,
  );
  if (typeof travelTime?.address === 'string' && travelTime.address.trim()) {
    listing.address = travelTime.address.trim();
  }
  if (Number.isFinite(travelTime?.location?.lat) && Number.isFinite(travelTime?.location?.lng)) {
    listing.latitude = travelTime.location.lat;
    listing.longitude = travelTime.location.lng;
  }
  const officialProvider = officialProviderFromScoutCompany(detailBody?.contact?.contactData?.agent?.company);
  if (officialProvider) listing.officialProvider = officialProvider;
  const objectNumber = String(detailBody?.adTargetingParameters?.obj_objectnumber ?? '').trim();
  if (officialProvider === 'gewobag' && /^\d{4}\/\d{5}\/\d{4}\/\d{4}$/.test(objectNumber)) {
    listing.providerLink = `https://www.gewobag.de/fuer-mietinteressentinnen/mietangebote/${objectNumber.replaceAll('/', '-')}/`;
  }
  if (officialProvider === 'berlinovo' && /^\d{4}\/\d{4}\/\d{1,4}$/.test(objectNumber)) {
    listing.providerLink = `https://www.berlinovo.de/de/wohnung-id/${objectNumber.replaceAll('/', '-')}`;
  }

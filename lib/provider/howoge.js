 ...
 * @returns {Promise<any[]>}
 */
async function getProjectListings(project) {
...
async function getListings(url) {
  ...
  try {
    ...
    const projectListings = (await Promise.all(projects.map(getProjectListings))).flat();
...
function applyBlacklist(listing) {
  return (
    !isOneOf(listing.title, appliedBlackList) &&
    !isOneOf(listing.description, appliedBlackList) &&
    !isOneOf(listing.address, appliedBlackList)
  );
}
...
export const init = (sourceConfig, blacklist) => {
  ...
  config.url = API_URL;
  refererUrl = toRefererUrl(sourceConfig.url);

 ...
 * @property {string} [officialProvider] Runtime-only canonical provider id read from Scout or a matched direct link.
 * @property {string} [providerLink] Runtime-only direct link to a uniquely matched official provider listing.
 * @property {string} [providerName] Runtime-only human-readable housing company/provider name when an aggregator exposes it.
 * @property {string} [wbsRequirement] Runtime-only structured WBS requirement text when a provider exposes it.
 * @property {string} [wbsSourceText] Runtime-only WBS-related text carried over from a uniquely matched direct-provider listing.

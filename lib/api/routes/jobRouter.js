...
import { recheckRecentAreaListings } from '../../services/jobs/areaRecheckService.js';
...
export default async function jobPlugin(fastify) {
  ...
  fastify.post('/:jobId/recheck-area', async (request, reply) => {
    const { jobId } = request.params;
    try {
      ...
      if (!doesJobBelongsToUser(job, request)) {
        return reply.code(403).send({ message: 'You are trying to change a job that is not associated to your user' });
      }
      if (settings.demoMode && !isAdmin(request)) {
        return reply.code(403).send({ message: 'Sorry, but you cannot recheck the Demo Job ;)' });
      }
      if (isJobRunning(jobId)) {
        return reply.code(409).send({ message: 'Job is already running' });
      }
      if (!job.spatialFilter?.features?.some((feature) => feature?.geometry?.type === 'Polygon')) {
        return reply.code(400).send({ message: 'Job has no polygon filter' });
      }

      const result = recheckRecentAreaListings(job);
      bus.emit('jobs:runOne', { jobId, skipAutoApply: true });
      return reply.code(202).send({ ...result, message: 'Area recheck accepted' });
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ message: 'Unexpected error rechecking job area' });
    }
  });

...
import { resolveApplicationChannel } from '../../services/application/applicationAutomation.js';
...
export default async function applicationPlugin(fastify) {
  fastify.post('/status/:jobId/:listingId', async (request, reply) => {
    ...
    const channel = await resolveApplicationChannel(job.notificationAdapter);
    ...
    if (!['queued', 'running', 'applied', 'failed', 'cancelled'].includes(status)) {

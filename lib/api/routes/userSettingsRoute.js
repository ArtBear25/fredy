...
import { normalizeAutoApplyRule } from '../../services/application/applicationAutomation.js';
...
export default async function userSettingsPlugin(fastify) {
  ...
  fastify.post('/auto-apply', async (request, reply) => {
    ...
    const raw = request.body?.auto_apply;
    if (raw == null || typeof raw !== 'object' || Array.isArray(raw)) {
      return reply.code(400).send({ error: 'auto_apply must be an object.' });
    }
    const normalized = normalizeAutoApplyRule(raw);
    const numericFields = ['maxPrice', 'minSize', 'minRooms'];
    if (
      numericFields.some(
        (field) => raw[field] != null && raw[field] !== '' && normalized[field] == null,
      ) ||
      (Array.isArray(raw.travelTimes) && normalized.travelTimes.length !== raw.travelTimes.length)
    ) {
      return reply.code(400).send({ error: 'Auto-apply contains an invalid numeric or travel-time criterion.' });
    }
    ...
    if (normalized.jobIds.some((jobId) => !ownedJobIds.has(jobId))) {
      return reply.code(400).send({ error: 'Auto-apply references an unknown job.' });
    }

    const addressLabels = new Set(
      getAddresses(getUserSettings(userId)).map((address) => String(address?.label ?? '').trim()).filter(Boolean),
    );
    if (normalized.travelTimes.some((criterion) => !addressLabels.has(criterion.label))) {
      return reply.code(400).send({ error: 'Auto-apply references an unknown travel-time destination.' });
    }

    upsertSettings({ auto_apply: normalized }, userId);
    return { success: true, auto_apply: normalized };
  });

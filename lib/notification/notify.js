 ...
 * @param {Object[]} [rawListings] Unformatted numeric data for machine consumers.
 * @returns {Promise<any>[]} One promise per adapter that was found.
 */
export const send = (serviceName, newListings, notificationConfig, jobKey, baseUrl, rawListings = newListings) => {
  //this is not being used in tests, therefore adapter are always set
  return resolveAdapters(notificationConfig, jobKey).map((a) =>
    a.send({
      serviceName,
      newListings: a.config.id === 'http' ? rawListings : newListings,
      notificationConfig,
      jobKey,
      baseUrl,
    }),

...
export async function getHttpOutbox() {
  if (!outbox) {
    const [{ default: connection }, { getJob }, { discoverLocalApplicationChannel }] = await Promise.all([
      import('../services/storage/SqliteConnection.js'),
      import('../services/storage/jobStorage.js'),
      import('../services/application/applicationAutomation.js'),
    ]);
    outbox ??= createHttpOutbox(connection.getConnection(), {
      resolveFields: async (jobKey, endpoint) => {
        const configured = getJob(jobKey)?.notificationAdapter?.find(
          (channel) => channel.id === 'http' && channel.fields.endpointUrl === endpoint,
        )?.fields;
        if (configured) return configured;
        const local = await discoverLocalApplicationChannel();
        return local?.fields?.endpointUrl === endpoint ? local.fields : undefined;
      },
    });
  }
  return outbox;

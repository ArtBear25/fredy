...
import { isApplicationEndpoint } from '../../services/application/applicationAutomation.js';
...
const mapListing = (listing, baseUrl) => ({
  ...
  ...(listing.applyRequested ? { applyRequested: true } : {}),
  ...(listing.applicationTrigger ? { applicationTrigger: listing.applicationTrigger } : {}),
  ...(listing.callbackUrl ? { callbackUrl: listing.callbackUrl } : {}),
...
export const send = async ({ serviceName, newListings, notificationConfig, jobKey, baseUrl, isTest = false }) => {
  ...
  const applicationEndpoint = isApplicationEndpoint(endpointUrl);
  const relevantListings = applicationEndpoint ? newListings.filter((listing) => listing.applyRequested === true) : newListings;
  if (applicationEndpoint && relevantListings.length === 0) {
    return { ok: true, skipped: true };
  }
  const listings = relevantListings.map((l) => mapListing(l, baseUrl));
  const body = {

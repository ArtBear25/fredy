...
export async function resolveDbPath(configuredPath) {
  ...
  const withinDir = resolved === baseDir || resolved.startsWith(baseDir + path.sep);
  if (!withinDir) {
    throw new Error(`The SQLite notification adapter may only write inside "${baseDir}". Got "${resolved}".`);
  }
  return resolved;
}

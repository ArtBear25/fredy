...
function buildHtmlBody(jobName, serviceName, o, baseUrl, maxLength = null) {
  ...
  if (o.providerLink) {
    ...
    if (maxLength == null) return `${fixed}${meta ? `\n${escapeHtml(meta)}` : ''}`;
    const budget = maxLength - fixed.length - (meta ? 1 : 0);
    const fittedMeta = fitEscapedText(meta, budget);
    return `${fixed}${fittedMeta ? `\n${fittedMeta}` : ''}`;
  }
  ...
  const escapedMeta =
    maxLength == null ? escapeHtml(meta) : fitEscapedText(meta, maxLength - fixedLength);
  return `${header}${titleLine}${escapedMeta}${fredyLink}`;
}
...
function buildPlainCaption(jobName, serviceName, o, baseUrl) {
  ...
  if (o.providerLink) {
    return (
      ...
      `Auf Scout24 öffnen: ${o.link || ''}${meta ? `\n${meta}` : ''}`
    ).slice(0, 4096);
  }

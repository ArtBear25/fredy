...
function applicationStatusLines(listing) {
  ...
  const lines = [];
  if (application.autoMatched) lines.push('⚡ Auto-Bewerbung erfüllt');
  if (application.state === 'running') lines.push('⏳ Bewerbung läuft');
  else if (application.state === 'applied') lines.push('✅ Beworben');
  else if (application.state === 'failed') lines.push('⚠️ Bewerbung fehlgeschlagen');
  else if (application.autoMatched && !application.workflowAvailable) {
    lines.push('Kein Bewerbungs-Workflow vorhanden');
  }
  return lines;
}
...
function applicationKeyboard(listing) {
  const application = listing?.application;
  if (!application?.workflowAvailable || ['running', 'applied'].includes(application.state)) return undefined;
  return {
...
function buildHtmlBody(jobName, serviceName, o, baseUrl, maxLength = null) {
  ...
  const escapedDetails =
    maxLength == null ? escapeHtml(details) : fitEscapedText(details, maxLength - fixedLength);

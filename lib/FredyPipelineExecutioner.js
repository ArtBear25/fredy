 ...
 * 8) Geocode listings and withhold provider-marked entries that still lack
 *    coordinates required by a spatial filter
 * 9) Persist new listings
 * 10) Filter out entries similar to already seen ones
 * 11) Filter out entries that do not match the job's specFilter
 * 12) Filter out entries that do not match the job's spatialFilter
 * 13) Dispatch notifications
 */
class FredyPipelineExecutioner {
   ...
   * step whenever a polygon filter is active so it can supply authoritative
   * coordinates before the spatial filter runs.
  ...
  async _fetchDetails(newListings) {
    ...
    if (!providerDetailsEnabled && !detailsAlwaysRequired && !detailsRequiredForSpatialFilter) {
      return newListings;
    }
  ...
  /**
   * Providers that promise authoritative coordinates for polygon filtering may
   * opt into strict handling. An unlocated listing is withheld before storage
   * and notification, so a temporary detail/geocoding failure is retried on the
   * next scan instead of either leaking through the area filter or being stored
   * permanently as already known.
   *

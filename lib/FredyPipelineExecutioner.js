...
import {
  ...
  getPendingAreaRecheckListings,
  getListingsByApplicationIdentity,
  setListingStatus,
  storeListings,
  updateListingApplication,
...
import { send as sendHttpNotification } from './notification/adapter/http.js';
...
} from './services/application/applicationAutomation.js';
 ...
 * 9) Geocode and withhold provider-marked entries that still lack coordinates
 *    required by a spatial filter
...
class FredyPipelineExecutioner {
   ...
   * @param {{maxDetailFetches?: number|null, deferNotification?: boolean, forceDetails?: boolean, forceMissingRoomDetails?: boolean, similarityIgnoredProviders?: string[], skipAutoApply?: boolean}} [options] Run options. `maxDetailFetches` caps how
  ...
  constructor(providerConfig, job, providerId, similarityCache, browser, options = {}) {
    ...
    /** @type {boolean} This run may notify, but must not start automatic applications. */
    this._skipAutoApply = options.skipAutoApply === true;
    /** @type {Set<string>} Queued history stays notification-only even in a later normal run. */
    this._areaRecheckHashes = new Set();
    /** @type {Object} */
    ...
    /** @type {boolean} */
    this._deferNotification = options?.deferNotification === true;
    /** @type {boolean} */
    this._forceDetails = options?.forceDetails === true;
    /** @type {boolean} */
    this._forceMissingRoomDetails = options?.forceMissingRoomDetails === true;
    /** @type {string[]} */
    this._similarityIgnoredProviders = Array.isArray(options?.similarityIgnoredProviders)
      ? options.similarityIgnoredProviders
      : [];
  }
  ...
  execute() {
    return (
      Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
        ...
        .then(this._includeAreaRechecks.bind(this))
        ...
        // Providers whose result pages expose only Warmmiete load the authoritative
        // Kaltmiete above. Re-apply the cheap spec filter to that enriched value.
        .then(this._filterBySpecs.bind(this))
        .then(this._filterAfterDetails.bind(this))
        .then(this._geocode.bind(this))
        .then(this._filterUnlocatedListings.bind(this))
   ...
   * Besides the user's provider-details setting, a provider may require this
   * step for authoritative prices or whenever a polygon filter is active.
  ...
  async _fetchDetails(newListings) {
    ...
    const enabledProviders = userId ? (getUserSettings(userId)?.provider_details ?? []) : [];
    const providerDetailsEnabled = Array.isArray(enabledProviders) && enabledProviders.includes(this._providerId);
    const polygonFeatures = this._jobSpatialFilter?.features?.filter((f) => f.geometry?.type === 'Polygon');
    const detailsAlwaysRequired = this._providerConfig.fetchDetailsAlways === true;
    const detailsRequiredForSpatialFilter =
      this._providerConfig.fetchDetailsForSpatialFilter === true && polygonFeatures?.length > 0;

    const detailsRequestedNormally =
      this._forceDetails || providerDetailsEnabled || detailsAlwaysRequired || detailsRequiredForSpatialFilter;
    const missingRoomListings = this._forceMissingRoomDetails
      ? newListings.filter((listing) => listing.rooms == null)
      : [];
    if (!detailsRequestedNormally && missingRoomListings.length === 0) {
      return newListings;
    }

    // The strict Scout24/provider matcher needs a room count even when the user's general detail
    // enrichment setting is off. In that narrow case enrich only the missing-room entries and keep
    // every already-complete listing in the batch untouched.
    if (!detailsRequestedNormally) {
      const listingsToEnrich =
        this._maxDetailFetches == null ? missingRoomListings : missingRoomListings.slice(0, this._maxDetailFetches);
      const replacements = new Map();
      for (const listing of listingsToEnrich) {
        replacements.set(listing, await this._providerConfig.fetchDetails(listing, this._browser));
      }
      return newListings.map((listing) => replacements.get(listing) ?? listing);
    }
  ...
  /**
   * Withhold listings that a provider promises to locate authoritatively but
   * that still have no usable coordinates. This happens before storage so a
   * temporary detail/geocoding failure can be retried on the next scan.
   *
   * @param {ParsedListing[]} listings
   * @returns {ParsedListing[]}
   */
  _filterUnlocatedListings(listings) {
    const polygonFeatures = this._jobSpatialFilter?.features?.filter((f) => f.geometry?.type === 'Polygon');
    if (!polygonFeatures?.length || this._providerConfig.requireCoordinatesForSpatialFilter !== true) {
      return listings;
    }
    ...
    const skipped = listings.length - locatedListings.length;
    if (skipped > 0) {
      logger.warn(
        `Withholding ${skipped} unlocated listing(s) from spatial filtering (Provider: '${this._providerId}').`,
      );
    }
  ...
  _filterByArea(newListings) {
    ...
    if (toDeleteListingByIds.length > 0) {
      deleteListingsById(toDeleteListingByIds, false, 'area');
    }
  ...
  /**
   * Merge explicitly queued historical offers with normalized search results.
   * @param {ParsedListing[]} listings Current normalized offers.
   * @returns {ParsedListing[]} Current offers and queued historical offers.
   */
  _includeAreaRechecks(listings) {
    const queued = getPendingAreaRecheckListings(this._jobKey, this._providerId);
    this._areaRecheckHashes = new Set(queued.map((listing) => listing.id));
    // Prefer freshly fetched fields, but still process queued stored offers when an archive
    // no longer includes them on its first page. All ordinary filters run after this merge.
    return [...new Map([...queued, ...listings].map((listing) => [listing.id, listing])).values()];
  }

  /** @param {object[]} listings Raw provider results. @returns {ParsedListing[]} */
  _normalize(listings) {
  ...
  async _notify(newListings) {
    if (this._deferNotification) return newListings;
    return this.notify(newListings);
  }

  /**
   * Dispatch a prepared listing batch for this provider. Job-level orchestration uses this after
   * strict Scout24/provider matching so the ordinary formatting and adapter fan-out stay in one
   * place rather than being reimplemented by the scheduler.
   *
   * @param {ParsedListing[]} newListings Listings to notify about.
   * @param {{skipApplications?:boolean, refreshTelegram?:boolean}} [options]
   * @returns {Promise<ParsedListing[]>}
   */
  async notify(newListings, { skipApplications = false, refreshTelegram = false } = {}) {
    if (newListings.length === 0) {
      throw new NoNewListingsWarning();
    }
    ...
    const applicationChannel = skipApplications
      ? null
      : await this._prepareApplications(newListings, job, userSettings, settings);
    const sendNotifications = refreshTelegram
      ? newListings.map((listing) => refreshApplicationMessages({ listing, job, baseUrl }))
      : notify.send(
          this._providerId,
          newListings.map((listing) =>
            formatListing(skipApplications ? { ...listing, rememberTelegramMessage: true } : listing, language),
          ),
    ...
    if (
      applicationChannel &&
      newListings.some((listing) => listing.applyRequested === true) &&
      (applicationChannel.autoDiscovered || refreshTelegram)
    ) {
      sendNotifications.push(
        sendHttpNotification({
          serviceName: this._providerId,
          newListings,
          notificationConfig: [applicationChannel],
          jobKey: this._jobKey,
          baseUrl,
        }),
      );
  ...
  /**
   * Decide application automation once, after travel times and strict Scout/direct matching are
   * complete but before any notification is sent. Matching the user's filter and being executable
   * are deliberately separate facts: a listing can satisfy every criterion while the provider has
   * no usable workflow. Only the conjunction (`autoEligible`) starts an application. Telegram can
   * therefore report a real blocker instead of calling a criteria match a successful auto-apply.
   *
   * @param {ParsedListing[]} listings
   * @param {Object|null} job
   * @param {Record<string, any>} userSettings
   * @param {Record<string, any>} settings
   * @returns {Promise<Object|null>} Application channel used for this batch, if available.
   * @private
   */
  async _prepareApplications(listings, job, userSettings, settings) {
    const applicationChannel = await resolveApplicationChannel(this._jobNotificationConfig);
    let workflows = new Set();
    let workflowLookupFailed = false;
    if (applicationChannel) {
      try {
        workflows = await fetchWorkflowProviders(applicationChannel);
      } catch (error) {
        workflowLookupFailed = true;
        logger.warn(`Could not read application workflows for job '${this._jobKey}': ${error.message}`);
      }
    }

    for (const listing of listings) {
      const provider = applicationProvider(this._providerId, listing);
      const url = applicationUrl(listing);
      const criteriaMatched = matchesAutoApplyRule(userSettings?.auto_apply, {
        jobId: this._jobKey,
        listing,
      });
      const wbs = evaluateWbsCompatibility(listing, applicationChannel?.applicantWbs);
      const additionalEligibility = evaluateAdditionalEligibility(listing, applicationChannel?.applicantEligibility);
      const workflowStatus = !applicationChannel
        ? 'module-missing'
        : workflowLookupFailed
          ? 'unreachable'
          : provider && workflows.has(provider)
            ? 'available'
            : 'missing';
      const workflowAvailable = workflowStatus === 'available';
      ...
      const previous = provider && url ? getListingsByApplicationIdentity(provider, url) : [];
      const autoApplySuppressed =
        this._skipAutoApply ||
        listing.areaRecheck === true ||
        previous.some((row) => row.application?.autoApplySuppressed === true);
      const autoEligible =
        !autoApplySuppressed &&
        criteriaMatched &&
        workflowAvailable &&
        wbs.compatible &&
        additionalEligibility.compatible;
      if (autoApplySuppressed) {
        listing.applyRequested = false;
        delete listing.applicationTrigger;
        delete listing.callbackUrl;
      }
      const alreadyApplied = previous.some(
        (row) => row.application?.state === 'applied' || row.status?.status === 'applied',
      );
      const alreadyRunning = previous.some((row) => row.application?.state === 'running');
      const alreadyQueued = previous.some((row) => row.application?.state === 'queued');
      const alreadyCancelled = previous.some((row) => row.application?.state === 'cancelled');
      const previousFailed = previous.some((row) => row.application?.state === 'failed');

      let state = alreadyApplied
        ? 'applied'
        : alreadyRunning
          ? 'running'
          : alreadyQueued
            ...
            : alreadyCancelled
              ...
              : previousFailed
                ? 'failed'
                : 'idle';
      let trigger = previous.find((row) => row.application?.trigger)?.application?.trigger ?? null;
      if (state === 'idle' && autoEligible) {
        state = 'queued';
        trigger = 'auto';
        listing.applyRequested = true;
        listing.applicationTrigger = 'auto';
        listing.callbackUrl = callbackUrl;
      }
      ...
      listing.application = {
        ...
        workflowStatus,
        workflowAvailable,
        wbsStatus: wbs.status,
        wbsLevels: wbs.levels,
        wbsCompatible: wbs.compatible,
        specialHousingNeedRequired: additionalEligibility.specialHousingNeedRequired,
        specialHousingNeedCompatible: additionalEligibility.specialHousingNeedCompatible,
        age55PlusRequired: additionalEligibility.age55PlusRequired,
        minimumAge: additionalEligibility.minimumAge,
        age55PlusCompatible: additionalEligibility.age55PlusCompatible,
        eligibilityCompatible: additionalEligibility.compatible,
        state,
        trigger,
      };
      if (provider && url) {
        listing.application = updateListingApplication(listing.id, listing.application) ?? listing.application;
      }
      if (state === 'applied') {
        setListingStatus(listing.id, 'applied');
      }
    }
  ...
  _save(newListings) {
    ...
    for (const listing of newListings) {
      if (this._areaRecheckHashes.has(listing.id)) listing.areaRecheck = true;
    }
   ...
   *
   * Public transport works from the same region-wide reachability answer the periodic sweep fetched,
   * which is cached for hours. For named transit addresses, one additional batched walking request
   * compares all new listings at once, so Telegram can show ÖPNV and Fußweg side by side without a
   * routing request per listing.
   *
  ...
  _filterBySimilarListings(listings) {
    ...
    const keptListings = listings.filter((listing) => {
      ...
      const similar = this._similarityIgnoredProviders.length
        ? this._similarityCache.checkAndAddEntry(similarityRecord, {
            ignoreProviders: this._similarityIgnoredProviders,
          })
        : this._similarityCache.checkAndAddEntry(similarityRecord);
      if (similar) {
        logger.debug(

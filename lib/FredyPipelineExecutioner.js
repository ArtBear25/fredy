...
class FredyPipelineExecutioner {
   ...
   * complete but before any notification is sent. Telegram therefore reports the decision while
   * the application HTTP delivery starts in the same fan-out, never after a user-facing wait.
   *
   * @param {ParsedListing[]} listings
  ...
  async _prepareApplications(listings, job, userSettings, settings) {
    ...
    for (const listing of listings) {
      ...
      const autoMatched = matchesAutoApplyRule(userSettings?.auto_apply, {
        jobId: this._jobKey,
        listing,
      });
      const workflowAvailable = Boolean(applicationChannel && provider && workflows.has(provider));
      ...
      const alreadyApplied = previous.some((row) => row.application?.state === 'applied' || row.status?.status === 'applied');
      ...
      if (state === 'idle' && autoMatched && workflowAvailable) {
        state = 'running';
        trigger = 'auto';

...
class FredyPipelineExecutioner {
  ...
  async notify(newListings) {
    ...
    const applicationChannel = await this._prepareApplications(newListings, job, userSettings, settings);
    ...
    if (applicationChannel?.autoDiscovered && newListings.some((listing) => listing.applyRequested === true)) {
  ...
  async _prepareApplications(listings, job, userSettings, settings) {
    ...
    for (const listing of listings) {
      ...
      if (provider && url) {
        updateListingApplication(listing.id, listing.application);
      }
      if (state === 'applied') {

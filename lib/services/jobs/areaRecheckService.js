 ...
 *
 * Visible listings outside the polygon are hidden. Every hidden listing inside it is queued
 * for the normal provider pipeline, including manual deletions and legacy exclusions.
 * The explicit button press authorizes this retry; no rows are deleted.
 *
 * @param {Object} job
 * @param {string} job.id
...
export function recheckRecentAreaListings(job, options = {}) {
  ...
  for (const listing of candidates) {
    ...
    if (inside && hidden && !listing.area_recheck_pending) {

...
export function initJobExecutionService({ providers, intervalMs }) {
  ...
  async function executeJob(job) {
    ...
    try {
      ...
      const deferredProviderIds = strictMergeActive
        ? new Set(['immoscout', ...activeOfficialProviderIds])
        : new Set();
      ...
      for (const prov of jobProviders) {
        try {
          ...
          const deferNotification = deferredProviderIds.has(prov.id);
          ...
          const pipeline = new FredyPipelineExecutioner(providerConfig, job, prov.id, similarityCache, browser, {
            deferNotification,
            forceMissingRoomDetails: strictMergeActive && prov.id === 'immoscout',
      ...
      if (strictMergeActive) {
        ...
        const merged = mergeScoutWithOfficialListings(scoutRun?.listings ?? [], officialBatches);
        const notificationBatches = new Map([['immoscout', merged.scoutListings]]);
        for (const providerId of activeOfficialProviderIds) {
          notificationBatches.set(providerId, merged.remainingByProvider.get(providerId) ?? []);
        }

        // Preserve the configured provider order for notification timing. A failed provider run is
        // simply absent from deferredRuns; Scout results still go out without a guessed direct link.
        for (const prov of jobProviders) {

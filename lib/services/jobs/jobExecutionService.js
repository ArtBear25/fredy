  ...
  async function executeJob(job, options = {}) {
    ...
    try {
      ...
      if (strictMergeActive) {
        // Scout goes first so its structured advertiser is available immediately; only the direct
        // providers needed for the merge run before we notify. Slow unrelated portals run later.
        const scoutProvider = jobProviders.find((prov) => prov.id === 'immoscout');
        const priorityProviders = [
          scoutProvider,
          ...jobProviders.filter((prov) => activeOfficialProviderIds.includes(prov.id)),
        ].filter(Boolean);
        for (const prov of priorityProviders) {
        ...
        // were already known before this cycle. These become a new combined notification rather
        // than a standalone InBerlinWohnen/direct-provider duplicate.
        ...
        const notificationBatches = new Map([['immoscout', [...sameRunMerge.scoutListings, ...delayedScoutMatches]]]);
        for (const providerId of activeOfficialProviderIds) {
          notificationBatches.set(providerId, delayedMerge.remainingByProvider.get(providerId) ?? []);
        }

        // Notify as soon as the Scout/direct-provider phase is complete. This is intentionally
        // before Immowelt, Kleinanzeigen and the other ordinary providers.
        for (const prov of priorityProviders) {
          const run = deferredRuns.get(prov.id);
          const listings = notificationBatches.get(prov.id) ?? [];

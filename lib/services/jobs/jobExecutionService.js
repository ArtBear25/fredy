...
import { getActiveListingsForJobAndProvider } from '../storage/listingsStorage.js';
import { isDemoJob } from '../demo/demoService.js';
import { runConnectivity } from '../crons/connectivity-cron.js';
import {
  STRICT_OFFICIAL_PROVIDER_IDS,
  mergeScoutWithOfficialListings,
} from '../similarity-check/listingFingerprint.js';
...
export function initJobExecutionService({ providers, intervalMs }) {
   ...
   *
   * @param {string} jobId
   * @param {{skipAutoApply?: boolean}} [options] Suppress automatic applications for this run only.
   * @returns {Promise<void>}
   */
  async function runSingle(jobId, options = {}) {
    const liveSettings = await getSettings();
  ...
  async function executeJob(job, options = {}) {
    ...
    try {
      ...
      const configuredProviderIds = new Set(jobProviders.map((provider) => provider.id));
      const activeOfficialProviderIds = STRICT_OFFICIAL_PROVIDER_IDS.filter((id) => configuredProviderIds.has(id));
      const strictMergeActive = configuredProviderIds.has('immoscout') && activeOfficialProviderIds.length > 0;
      const deferredProviderIds = strictMergeActive ? new Set(['immoscout', ...activeOfficialProviderIds]) : new Set();
      // Snapshot before this run stores its new Scout results. Anything in here was already known
      // when the cycle started and can therefore receive a direct link that appeared later.
      const knownScoutListings = strictMergeActive ? getActiveListingsForJobAndProvider(job.id, 'immoscout') : [];
      const deferredRuns = new Map();

      const runProvider = async (prov, deferNotification = false) => {
        try {
          ...
          const similarityIgnoredProviders =
            prov.id === 'immoscout'
              ? activeOfficialProviderIds
              : activeOfficialProviderIds.includes(prov.id)
                ? ['immoscout']
                : [];
          const pipeline = new FredyPipelineExecutioner(providerConfig, job, prov.id, similarityCache, browser, {
            skipAutoApply: options.skipAutoApply === true,
            deferNotification,
            forceDetails: strictMergeActive && prov.id === 'immoscout',
            forceMissingRoomDetails: false,
            similarityIgnoredProviders: deferNotification ? similarityIgnoredProviders : [],
          });
          const listings = await pipeline.execute();
          if (deferNotification) {
            deferredRuns.set(prov.id, { pipeline, listings: Array.isArray(listings) ? listings : [] });
          }
        } catch (err) {
          logger.error(err);
        }
      ...
      if (strictMergeActive) {
        const scoutProvider = jobProviders.find((prov) => prov.id === 'immoscout');
        const officialProviders = jobProviders.filter((prov) => activeOfficialProviderIds.includes(prov.id));

        // Get Scout out immediately. Provider matching is enrichment, not a reason to hold the
        // user's first notification hostage. Application handling waits until the direct-provider
        // phase below, so a temporary Scout-only message cannot start the wrong workflow.
        if (scoutProvider) {
          await runProvider(scoutProvider, true);
          const scoutRun = deferredRuns.get('immoscout');
          if (scoutRun?.listings.length) {
            try {
              await scoutRun.pipeline.notify(scoutRun.listings, { skipApplications: true });
            } catch (err) {
              logger.error(err);
            }
          }
        ...
        for (const prov of officialProviders) {
          await runProvider(prov, true);
        }

        const scoutRun = deferredRuns.get('immoscout');
        const officialBatches = activeOfficialProviderIds.map((providerId) => ({
          providerId,
          listings: deferredRuns.get(providerId)?.listings ?? [],
        }));

        // First consume direct listings that match Scout entries found in this same cycle.
        const sameRunMerge = mergeScoutWithOfficialListings(scoutRun?.listings ?? [], officialBatches);
        const stillUnmatchedOfficialBatches = activeOfficialProviderIds.map((providerId) => ({
          providerId,
          listings: sameRunMerge.remainingByProvider.get(providerId) ?? [],
        }));

        // Then give only the still-unmatched direct listings a chance to enrich Scout entries that
        // were already known before this cycle.
        const delayedMerge = mergeScoutWithOfficialListings(knownScoutListings, stillUnmatchedOfficialBatches);
        const delayedScoutMatches = delayedMerge.scoutListings.filter((listing) => listing.providerLink);

        // Reuse the original Scout message. This second pass only enriches that message and prepares
        // the correct provider workflow; it does not send a duplicate Telegram notification.
        const scoutUpdates = [
          ...sameRunMerge.scoutListings.filter((listing) => listing.providerLink),
          ...delayedScoutMatches,
        ];
        if (scoutRun && scoutUpdates.length) {
          try {
            await scoutRun.pipeline.notify(scoutUpdates, { refreshTelegram: true });
          } catch (err) {
            logger.error(err);
          }
        }

        // Official listings that were not consumed by a Scout match remain ordinary new listings.
        for (const prov of officialProviders) {
          const run = deferredRuns.get(prov.id);
          const listings = delayedMerge.remainingByProvider.get(prov.id) ?? [];
          if (!run || listings.length === 0) continue;
          try {
            await run.pipeline.notify(listings);
          } catch (err) {
            logger.error(err);
          }
        }
        ...
        for (const prov of jobProviders) {
          if (deferredProviderIds.has(prov.id)) continue;
          await runProvider(prov, false);
        }
      } else {
        for (const prov of jobProviders) {
          await runProvider(prov, false);

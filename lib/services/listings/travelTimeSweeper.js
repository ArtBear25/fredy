...
function toEstimateRow(
  ...
  const row = {
    ...
    bikeMinutes: null,
    bikeGeometry: null,
    // New-listing notifications can cheaply batch one walking comparison for every listing from
    // this saved address. The periodic sweep leaves this null and keeps its no-per-listing cost.
    walkMinutes: comparisonWalk?.minutes ?? null,
    walkGeometry: null,
...
async function updateListing(listing, origins, { now, staleBefore, streetBudget, poiBudget, poiEnabled = true }) {
  ...
  for (const origin of origins) {
    ...
    const comparisonWalk = origin.comparisonWalks?.get(listing.id) ?? null;
    if (sameLabel && stillValid(sameLabel, origin.coords, origin.referenceTime, staleBefore, origin.mode)) {
      ...
      // which a fresh estimate would otherwise write over with a rougher number. A notification-time
      // walking comparison may enrich an older estimate without replacing its transit answer.
      const carried = carryOver(sameLabel, origin.label);
      if (carried.walkMinutes == null && comparisonWalk?.minutes != null) {
        carried.walkMinutes = comparisonWalk.minutes;
      }
    ...
    if (renamed) {
      const carried = carryOver(renamed, origin.label);
      if (carried.walkMinutes == null && comparisonWalk?.minutes != null) {
        carried.walkMinutes = comparisonWalk.minutes;
      }
      entries.push(carried);
    ...
    entries.push(
      toEstimateRow(origin.label, origin.coords, origin.referenceTime, transit, now, street, origin.mode, comparisonWalk),
    );
...
/**
 * Add a direct walking comparison for named addresses to a batch of new listings.
 *
 * The one-to-many street endpoint keeps this cheap: one walking request per saved transit address,
 * however many new listings are in the batch. The periodic sweep deliberately does not call this;
 * the comparison exists so a fresh Telegram notification can say, for example,
 * "DKB: 33 min Ö // 47 min F" without falling back to one routing request per flat.
 *
 * @param {Array<Object>} origins Resolved saved addresses.
 * @param {Array<{id: string, latitude: number, longitude: number}>} listings Located new listings.
 * @returns {Promise<void>}
 */
async function addNotificationWalkingComparisons(origins, listings) {
  const groups = listings.map((listing) => ({
    label: listing.id,
    candidates: [{ lat: listing.latitude, lng: listing.longitude }],
  }));
  if (groups.length === 0) return;

  for (const origin of origins) {
    if (origin.category != null || origin.mode !== 'transit' || origin.coords == null) continue;
    try {
      const result = await getPlaceMatrix({
        mode: 'walk',
        fromLat: origin.coords.lat,
      ...
      if (result.ok === true) {
        origin.comparisonWalks = result.matches;
      }
    } catch (error) {
      // Walking is a comparison, never a condition for notifying. Transit can still be shown when
      // this optional batch lookup is unavailable.
      logger.debug(`Could not calculate walking comparisons for '${origin.label}': ${error?.message || error}`);
    }
  }
}
 ...
 * long the commute is. Public transport uses the cached reachability answer; named transit addresses
 * additionally get one batched walking-comparison request per address, not per listing.
 *
...
export async function updateTravelTimesForListings(listings, addresses, { now = Date.now() } = {}) {
  ...
  try {
    ...
    await addNotificationWalkingComparisons(resolved, routable);

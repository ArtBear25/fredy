...
/**
 * The hard Scout24-to-official-provider comparison. Every required scalar has to be present and
 * agree; living space is the one optional signal and is only compared when both listings expose it.
 *
 * @param {any} scout
 * @param {any} official
 * @returns {boolean}
...
export function isStrictScoutProviderMatch(scout, official) {
  ...
  if (
    scoutAddress.street == null ||
    scoutAddress.houseNumber == null ||
    scoutAddress.zip == null ||
    officialAddress.street == null ||
    officialAddress.houseNumber == null ||
    officialAddress.zip == null ||
  ...
  const scoutRooms = Number(scout?.rooms);
  const officialRooms = Number(official?.rooms);
  if (!Number.isFinite(scoutRooms) || !Number.isFinite(officialRooms) || scoutRooms !== officialRooms) return false;

  const scoutPrice = Number(scout?.price);
  const officialPrice = Number(official?.price);
  if (!Number.isFinite(scoutPrice) || !Number.isFinite(officialPrice) || Math.abs(scoutPrice - officialPrice) > 1) {
    return false;
  }

  const scoutSize = scout?.size == null ? null : Number(scout.size);
  const officialSize = official?.size == null ? null : Number(official.size);
  if (scoutSize != null && officialSize != null) {
    if (!Number.isFinite(scoutSize) || !Number.isFinite(officialSize) || Math.abs(scoutSize - officialSize) > 1) {
      return false;
    }
  }
...
export function mergeScoutWithOfficialListings(scoutListings = [], officialBatches = []) {
  const candidates = officialBatches.flatMap(({ providerId, listings = [] }) =>
    listings.map((listing, index) => ({
      key: `${providerId}:${index}`,
      providerId,
      listing,
      directLink: strictDirectProviderLink(providerId, listing?.link),
    })),
  );
  const matchesByScout = scoutListings.map((scout) =>
    candidates.filter((candidate) => candidate.directLink && isStrictScoutProviderMatch(scout, candidate.listing)),
  );
  ...
  const mergedScouts = scoutListings.map((scout, index) => {
    ...
    consumed.add(matches[0].key);
    return { ...scout, providerLink: matches[0].directLink };
  });

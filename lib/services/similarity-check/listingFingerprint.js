...
export const STRICT_OFFICIAL_PROVIDER_IDS = Object.freeze([
  'degewo',
  'howoge',
  'wbm',
  'gewobag',
...
const GEWOBE_PORTAL_HOST = 'portal.immobilienscout24.de';
const GEWOBE_PORTAL_ACCOUNT_ID = '54212102';
...
const STRICT_DIRECT_HOSTS = new Map([
  ...
  ['wbm', new Set(['wbm.de'])],
  ['gewobag', new Set(['gewobag.de'])],
  ['gewobe', new Set([GEWOBE_PORTAL_HOST])],
  [
    'inberlinwohnen',
...
export function strictDirectProviderLink(providerId, link) {
  ...
  try {
    ...
    if (
      providerId === 'gewobe' &&
      !new RegExp(`^/expose/${GEWOBE_PORTAL_ACCOUNT_ID}/\\d+/`).test(url.pathname)
    ) {
      return null;
    }
    return url.href;
...
export function officialProviderFromDirectLink(link) {
  ...
  try {
    ...
    if (
      hostname === GEWOBE_PORTAL_HOST &&
      new RegExp(`^/expose/${GEWOBE_PORTAL_ACCOUNT_ID}/\\d+/`).test(url.pathname)
    ) {
      return 'gewobe';
    }
    return OFFICIAL_PROVIDER_BY_HOST.get(hostname) ?? null;
  } catch {
    return null;
  }
}
...
export function mergeScoutWithOfficialListings(scoutListings = [], officialBatches = []) {
  ...
  const mergedScouts = scoutListings.map((scout, index) => {
    ...
    return {
      ...
      ...(matches[0].listings.some((listing) => listing.areaRecheck === true) ? { areaRecheck: true } : {}),

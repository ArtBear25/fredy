...
/**
 * Provider counts for each exact hash. The generic cache normally only needs the aggregate count,
 * but a run that performs the stricter Scout24↔official-provider merge must be able to ignore only
 * that counterpart while still recognising an exact duplicate from every other provider.
 * @type {Map<string, Map<string, number>>}
 */
let exactProviders = new Map();
...
export const initSimilarityCache = () => {
  ...
  for (const entry of allEntries) {
    ...
    addExactProvider(newProviders, hash, record.provider);
...
/** @param {string|null|undefined} provider @returns {string} */
function providerKey(provider) {
  return provider == null ? UNKNOWN_PROVIDER : String(provider);
}

/**
 * Increment one provider's contribution to an exact hash.
 * @param {Map<string, Map<string, number>>} target
 * @param {string} hash
 * @param {string|null|undefined} provider
 */
function addExactProvider(target, hash, provider) {
  const providers = target.get(hash) ?? new Map();
  const key = providerKey(provider);
  providers.set(key, (providers.get(key) ?? 0) + 1);
  target.set(hash, providers);
}

/**
 * Whether an exact hash is represented by at least one provider this run did not ask the generic
 * matcher to ignore.
 * @param {string} hash
 * @param {Set<string>} ignoredProviders
 * @returns {boolean}
 */
function hasUsableExactMatch(hash, ignoredProviders) {
  const providers = exactProviders.get(hash);
  if (providers == null) return exactHashes.has(hash);
  for (const [provider, count] of providers) {
    if (count > 0 && !ignoredProviders.has(provider)) return true;
  }
  return false;
}

/**
 * Remove one provider contribution from an exact hash. A missing provider is supported for legacy
 * callers/tests by removing one arbitrary contribution, matching the old aggregate-count behavior.
 * @param {string} hash
 * @param {string|null|undefined} provider
 */
function removeExactProvider(hash, provider) {
  const providers = exactProviders.get(hash);
  if (providers == null) return;
  let key = provider == null ? providers.keys().next().value : providerKey(provider);
  if (!providers.has(key) && provider != null) key = providers.keys().next().value;
  if (key == null) return;
  const count = providers.get(key) ?? 0;
  if (count <= 1) providers.delete(key);
  else providers.set(key, count - 1);
  if (providers.size === 0) exactProviders.delete(hash);
}
 ...
 * @param {{ignoreProviders?: string[]}} [options] Providers whose comparison against this record is
 *   deferred to a stricter run-level matcher. Every other cached provider remains active.
 * @returns {boolean} true if the entry was already known (duplicate), otherwise false
 */
export const checkAndAddEntry = (record, options = {}) => {
  const ignoredProviders = new Set(Array.isArray(options.ignoreProviders) ? options.ignoreProviders : []);
  const { jobId, title, address, price } = record ?? {};
  const hash = toHash(jobId, title, price, address);
  if (exactHashes.has(hash) && hasUsableExactMatch(hash, ignoredProviders)) {
    return true;
  }
  ...
  for (const key of candidateBlockKeys(fingerprint)) {
    ...
    for (const known of bucket) {
      if (ignoredProviders.has(known.provider)) continue;
      if (isLikelyDuplicate(fingerprint, known)) return true;
    }
  }

  exactHashes.set(hash, (exactHashes.get(hash) ?? 0) + 1);
  addExactProvider(exactProviders, hash, record?.provider);
...
export const removeEntry = (record) => {
  ...
  removeExactProvider(hash, record?.provider);
  ...
  if (fingerprint.blockKey != null) {
    ...
    const index =
      bucket?.findIndex(
        (known) => known.hash === hash && (record?.provider == null || known.provider === record.provider),
      ) ?? -1;
    if (index >= 0) {

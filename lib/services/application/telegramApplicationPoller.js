...
import { buildApplicationEvent, fetchWorkflowProviders, resolveApplicationChannel } from './applicationAutomation.js';
...
export async function handleApplicationCallback({ token, callbackQuery, fetchImpl = (...args) => fetch(...args) }) {
  ...
  const telegramChannel = job?.notificationAdapter?.find(
    (channel) =>
      channel.id === 'telegram' && channel.fields?.token === token && chatIds(channel).includes(callbackChatId),
  );
  ...
  if (identityRows.some((row) => row.application?.state === 'queued')) {
    await answerCallback(token, callbackId, 'Bewerbung wartet bereits', fetchImpl);
  ...
  if (identityRows.some((row) => row.application?.state === 'cancelled')) {
    await answerCallback(token, callbackId, 'Im Bewerbungsmodul neu einreihen', fetchImpl);
  ...
  await answerCallback(token, callbackId, 'Bewerbung eingereiht', fetchImpl);

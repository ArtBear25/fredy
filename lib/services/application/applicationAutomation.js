...
const AGE_55_PLUS_REQUIRED = [
  /\b(?:ab|mindestens)\s+(?:einem\s+alter\s+von\s+)?55\s*(?:jahre(?:n)?|j\.)\b/i,
  /\bmindestalter\s*(?::|=|-)?\s*55\s*(?:jahre?)?\b/i,
  /\b55\s*jahre?\s*(?:oder|und)\s+älter\b/i,
  /\b55\s*\+\s*(?:jahre?)?\b/i,
];
 ...
 * @returns {{specialHousingNeedRequired:boolean,specialHousingNeedCompatible:boolean,age55PlusRequired:boolean,age55PlusCompatible:boolean,compatible:boolean}}
 */
export function evaluateAdditionalEligibility(listing, applicantEligibility) {
  ...
  const age55PlusRequired = AGE_55_PLUS_REQUIRED.some((pattern) => pattern.test(text));

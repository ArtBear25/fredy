...
import { removeEntry, initSimilarityCache } from '../similarity-check/similarityCache.js';
...
const parseListingStatus = (row) => {
  ...
  if (typeof row.application_state === 'string') {
    row.application = fromJson(row.application_state, null);
  } else if (row.application_state == null && !('application' in row)) {
    row.application = null;
  }
  delete row.application_state;
  return row;
};
  ...
  return SqliteConnection.query(
    `SELECT hash
     ...
     WHERE job_id = @jobId
       ...
       AND NOT (manually_deleted = 1 AND COALESCE(exclusion_reason, '') = 'area' AND area_recheck_pending = 1 AND COALESCE(is_active, 0) = 1)`,
    { jobId, providerId },
  ).map((r) => r.hash);
};

/**
 * Queue hidden polygon candidates regardless of why they were hidden, retaining their records.
 * @param {string[]} ids Listing IDs selected against the current polygon.
 * @returns {{changes:number}|undefined}
 */
export const queueAreaRecheck = (ids) => {
  const result = forEachIdChunk(ids, (chunk, db) =>
    db
      .prepare(
        `UPDATE listings SET area_recheck_pending = 1, exclusion_reason = 'area'
     WHERE id IN (${chunk.map(() => '?').join(',')}) AND manually_deleted = 1
       AND is_active = 1 AND area_recheck_pending = 0`,
      )
      .run(chunk),
  );
  // Manual deletion can leave an old fingerprint in memory. Rebuild from visible rows so
  // retrying the hidden row does not mistake it for its own duplicate.
  if (result?.changes) initSimilarityCache();
  return result;
};

/**
 * Read queued stored offers in normalized form, independent of search-page pagination.
 * @param {string} jobId
 * @param {string} providerId
 * @returns {import('../../types/listing.js').ParsedListing[]}
 */
export const getPendingAreaRecheckListings = (jobId, providerId) =>
  SqliteConnection.query(
    `SELECT hash AS id, title, price, size, rooms, address, link, image_url AS image,
          description, latitude, longitude, build_year AS buildYear, energy_class AS energyClass
   FROM listings WHERE job_id = @jobId AND provider = @providerId
     AND manually_deleted = 1 AND area_recheck_pending = 1 AND is_active = 1`,
    { jobId, providerId },
  );

/**
 * Return recent, still-active listings of a job that have usable coordinates.
 *
 * The area recheck uses stored coordinates rather than geocoding hundreds of historical rows again.
 * Both visible and soft-deleted rows are returned: shrinking a polygon must hide visible rows that
 * no longer belong, while expanding one must let previously hidden rows be rediscovered.
 *
 * @param {string} jobId
 * @param {number} createdAfter Unix timestamp in milliseconds.
 * @returns {Array<{id:string, latitude:number, longitude:number, manually_deleted:number}>}
 */
export const getRecentAreaRecheckCandidates = (jobId, createdAfter) => {
  return SqliteConnection.query(
    `SELECT id, latitude, longitude, manually_deleted, exclusion_reason, area_recheck_pending
     FROM listings
     WHERE job_id = @jobId
       AND is_active = 1
...
/**
 * Return the currently active stored listings of one provider in one job. The strict Scout/direct
 * matcher uses this snapshot before a run so a direct-provider listing that appears later can be
 * paired with the Scout listing Fredy already announced in an earlier cycle.
 *
 * @param {string} jobId
 * @param {string} providerId
 * @returns {Object[]}
 */
export const getActiveListingsForJobAndProvider = (jobId, providerId) => {
 ...
 * - Existing rows stay unchanged, except explicitly queued polygon retries. Those are refreshed in place.
...
export const storeListings = (jobId, providerId, listings) => {
  ...
  SqliteConnection.withTransaction((db) => {
    ...
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, rooms, build_year, energy_class, title,
       ...
       ON CONFLICT(job_id, hash) DO UPDATE SET
         price = excluded.price, size = excluded.size, rooms = excluded.rooms,
         build_year = excluded.build_year, energy_class = excluded.energy_class,
         title = excluded.title, image_url = excluded.image_url, description = excluded.description,
         link = excluded.link,
         address = CASE WHEN listings.address_is_manual = 1 THEN listings.address ELSE excluded.address END,
         latitude = CASE WHEN listings.address_is_manual = 1 THEN listings.latitude ELSE excluded.latitude END,
         longitude = CASE WHEN listings.address_is_manual = 1 THEN listings.longitude ELSE excluded.longitude END,
         manually_deleted = 0, exclusion_reason = NULL, area_recheck_pending = 0
       WHERE listings.manually_deleted = 1 AND listings.exclusion_reason = 'area'
         AND listings.area_recheck_pending = 1 AND listings.is_active = 1
       RETURNING id, address, latitude, longitude`,
    );
    const findExisting = db.prepare(
      `SELECT id, manually_deleted, is_active FROM listings WHERE job_id = @job_id AND hash = @hash LIMIT 1`,
    );
    ...
    for (const item of listings) {
      ...
      const existing = inserted ? null : findExisting.get({ job_id: jobId, hash: params.hash });
      item.id = inserted?.id ?? existing?.id ?? params.id;
      // A user may delete a queued row while details are loading. Honour that decision even
      // though the listing passed _findNew before the deletion.
      if (existing?.manually_deleted || existing?.is_active === 0) withheld.add(item);
      if (inserted) {
        item.address = inserted.address;
        item.latitude = inserted.latitude;
        item.longitude = inserted.longitude;
      }
    }
    for (let index = listings.length - 1; index >= 0; index--) {
      if (withheld.has(listings[index])) listings.splice(index, 1);
    }
  });
...
export const deleteListingsByJobId = (jobId, hardDelete = false) => {
  ...
  return SqliteConnection.execute(
    `UPDATE listings
     SET manually_deleted = 1, exclusion_reason = 'other', area_recheck_pending = 0
     WHERE job_id = @jobId`,
    { jobId },
  );
};
 ...
 * @param {boolean} [hardDelete=false] - Whether to hard delete from DB or just mark as deleted.
 * @param {string} [reason=other] - Only the polygon filter may pass area.
 * @returns {any} The result from SqliteConnection.execute.
 */
export const deleteListingsById = (ids, hardDelete = false, reason = 'other') => {
  if (!Array.isArray(ids) || ids.length === 0) return;
  if (hardDelete) {
    let changes = 0;
    for (let offset = 0; offset < ids.length; offset += ID_CHUNK_SIZE) {
  ...
  const result = forEachIdChunk(ids, (chunk, db) => {
    // Polygon filtering follows insertion into the similarity cache. Evict only newly hidden rows
    // so a later recheck cannot collide with its own old fingerprint in this same process.
    const rows =
      reason === 'area'
        ? db
            .prepare(`SELECT * FROM listings WHERE manually_deleted = 0 AND id IN (${chunk.map(() => '?').join(',')})`)
            .all(chunk)
        : [];
    const updated = db
      .prepare(
        `UPDATE listings SET manually_deleted = 1, exclusion_reason = ?, area_recheck_pending = 0
       WHERE id IN (${chunk.map(() => '?').join(',')})${reason === 'area' ? ' AND manually_deleted = 0' : ''}`,
      )
      .run([reason, ...chunk]);
    for (const row of rows) {
      removeEntry({ ...row, jobId: row.job_id });
    }
    return updated;
  });
  return result;
...
export const restoreListingsById = (ids) => {
  return forEachIdChunk(ids, (chunk, db) =>
    db
      .prepare(
        `UPDATE listings
         SET manually_deleted = 0, exclusion_reason = NULL, area_recheck_pending = 0
         WHERE id IN (${chunk.map(() => '?').join(',')})`,
      )
      .run(chunk),
  );
...
/**
 * Merge application automation metadata into one listing. This is deliberately one JSON object:
 * transient progress, the provider/exposé identity and Telegram message references change together
 * and do not need their own query columns.
 *
 * @param {string} id
 * @param {Record<string, any>} patch
 * @returns {Object|null} The stored application state.
 */
export const updateListingApplication = (id, patch) => {
  if (!id || patch == null || typeof patch !== 'object') return null;
  const row = SqliteConnection.query(`SELECT application_state FROM listings WHERE id = @id LIMIT 1`, { id })[0];
  if (!row) return null;
  const current = fromJson(row.application_state, {}) ?? {};
  const application = { ...current, ...patch, updatedAt: Date.now() };
  SqliteConnection.execute(`UPDATE listings SET application_state = @state WHERE id = @id`, {
    id,
...
/**
 * Remember where a listing notification was posted so later application callbacks can edit that
 * exact Telegram message rather than sending a second status message.
 *
 * @param {string} id
 * @param {{configuredAdapterId:string, chatId:string|number, messageId:number, kind:'text'|'photo'}} ref
 * @returns {Object|null}
 */
export const addListingTelegramMessage = (id, ref) => {
  const row = SqliteConnection.query(`SELECT application_state FROM listings WHERE id = @id LIMIT 1`, { id })[0];
  if (!row || !ref?.configuredAdapterId || ref?.messageId == null) return null;
  const current = fromJson(row.application_state, {}) ?? {};
  const telegramMessages = Array.isArray(current.telegramMessages) ? [...current.telegramMessages] : [];
  const key = `${ref.configuredAdapterId}:${ref.chatId}:${ref.messageId}`;
  if (!telegramMessages.some((item) => `${item.configuredAdapterId}:${item.chatId}:${item.messageId}` === key)) {
    telegramMessages.push(ref);
  }
  return updateListingApplication(id, { telegramMessages });
};

/**
 * Return all Fredy rows that represent the same concrete provider exposé. Jobs are intentionally
 * absent from the identity: two searches may find the same flat, but it is still one application.
 *
 * @param {string} provider
 * @param {string} url
 * @returns {Object[]}
 */
export const getListingsByApplicationIdentity = (provider, url) => {
  if (!provider || !url) return [];
  return SqliteConnection.query(
    `SELECT l.*
     FROM listings l
     WHERE json_extract(l.application_state, '$.provider') = @provider
       AND json_extract(l.application_state, '$.url') = @url
       AND l.manually_deleted = 0
     ORDER BY l.created_at ASC`,
    { provider: String(provider).toLowerCase(), url },
  ).map(parseListingStatus);
};

/**
 * Merge one status update into every job row for the same application identity while preserving
 * row-specific Telegram message ids.
 *
 * @param {string} provider
 * @param {string} url
 * @param {Record<string, any>} patch
 * @returns {Object[]} Updated listing rows.
 */
export const updateApplicationByIdentity = (provider, url, patch) => {
  const rows = getListingsByApplicationIdentity(provider, url);
  for (const row of rows) {
    updateListingApplication(row.id, patch);
  }
  return getListingsByApplicationIdentity(provider, url);
};

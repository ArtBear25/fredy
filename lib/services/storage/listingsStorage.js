...
/**
 * Queue only confirmed polygon exclusions, preserving row IDs, notes and related records.
...
export const queueAreaRecheck = (ids) =>
  forEachIdChunk(ids, (chunk, db) =>
    db
      .prepare(
        `UPDATE listings SET area_recheck_pending = 1
    WHERE id IN (${chunk.map(() => '?').join(',')}) AND manually_deleted = 1
      AND exclusion_reason = 'area' AND is_active = 1 AND area_recheck_pending = 0`,
      )
      .run(chunk),
  );
  ...
  return SqliteConnection.query(
    `SELECT id, latitude, longitude, manually_deleted, exclusion_reason, area_recheck_pending
     ...
     WHERE job_id = @jobId
       ...
       AND (manually_deleted = 0 OR exclusion_reason = 'area')

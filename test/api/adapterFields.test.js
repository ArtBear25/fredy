...
describe('privileged notification adapter fields', () => {
  ...
  describe('sqlite adapter path containment', () => {
    ...
    it('resolves a bare file name inside the database directory', async () => {
      ...
      expect(await resolveDbPath('notifications.db')).toBe(path.resolve('/tmp/fredy-db/notifications.db'));
    });

    it('defaults to listings.db', async () => {
      const { resolveDbPath } = await loadAdapter();
      expect(await resolveDbPath(undefined)).toBe(path.resolve('/tmp/fredy-db/listings.db'));
      expect(await resolveDbPath('   ')).toBe(path.resolve('/tmp/fredy-db/listings.db'));
    });
    ...
    it('allows a nested path below the database directory', async () => {
      const { resolveDbPath } = await loadAdapter();
      expect(await resolveDbPath('exports/2026.db')).toBe(path.resolve('/tmp/fredy-db/exports/2026.db'));
    });

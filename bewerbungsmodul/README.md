...
Im normalen lokalen Betrieb ist keine Einrichtung nötig: Bewerbungsmodul starten, danach erkennt Fredy den Dienst automatisch auf `127.0.0.1:8765` und verwendet dessen bestehenden Bearer-Token intern. Die lokale Erkennung übergibt außerdem nur die gespeicherte WBS-Angabe (`vorhanden` und Typ), damit Fredy eindeutig unpassende WBS-Treffer vor dem automatischen Start von Selenium aussortieren kann; ein manueller Telegram-Start bleibt davon unberührt. Ein manuell angelegter HTTP-Kanal bleibt als expliziter Override unterstützt. `BEWERBUNGSMODUL_PORT` kann für beide Prozesse einen anderen lokalen Port vorgeben.

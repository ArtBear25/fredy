...
Python 3.12 und ein Doppelklick auf `Bewerbungsmodul starten.cmd` genügen. Der Starter erstellt bei Bedarf `.venv` und installiert die festgeschriebenen Pakete aus `requirements-lock.txt` von PyPI. Unveränderte Pakete werden nicht erneut installiert. Der Starter öffnet ein eigenes sichtbares Konsolenfenster `Fredy Bewerbungsmodul`; `Strg+C` oder das Schließen dieses Fensters beendet den vollständigen Backend-Prozessbaum. Das Dashboard ist danach unter `http://127.0.0.1:8765` erreichbar, wird aber nicht ungefragt als neuer Browser-Tab geöffnet. Läuft dort bereits eine Instanz, wird sie nicht still weiterverwendet: der Starter zeigt den Konflikt im Konsolenfenster an.
...
Im normalen lokalen Betrieb ist keine Einrichtung nötig: Bewerbungsmodul starten, danach erkennt Fredy den Dienst automatisch auf `127.0.0.1:8765` und verwendet dessen bestehenden Bearer-Token intern. Die lokale Erkennung übergibt außerdem die gespeicherte WBS-Angabe (`vorhanden` und Typ) sowie die beiden Eligibility-Schalter `Besonderer Wohnbedarf` und `55 Jahre oder älter`. Fredy nutzt diese Angaben, um eindeutig unpassende Treffer vor dem automatischen Start von Selenium auszusortieren; unklare Fälle bleiben fail-open und ein manueller Telegram-Start bleibt davon unberührt. Ein manuell angelegter HTTP-Kanal bleibt als expliziter Override unterstützt. `BEWERBUNGSMODUL_PORT` kann für beide Prozesse einen anderen lokalen Port vorgeben.
